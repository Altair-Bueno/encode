...
mod tests {
    ...
    fn assert_that_be_u8_try_from_usize(#[case] val: usize, #[case] expected: Option<u8>) {
        let result: Result<u8, TryFromIntError> = BE::<u8>::try_from(val).map(|be| be.into_inner());
